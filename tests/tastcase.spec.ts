...
test('verify new user can create an account', async ({ page }) => {
  ...
  await page.fill('input[name="email"]', 'randika2000@gmail.com');
...
test('verify user can login system giving valied credentials', async ({ page }) => {
    // Step 1: Navigate to the Wiley Online Library homepage
    await page.goto('https://wiley.scienceconnect.io/login');
  
    // Step 3: Fill in all required fields
    await page.fill('input[name="email"]', 'piyumirandi6@gmail.com');
    ...
    await page.fill('input[name="password"]', 'piyumi000');
  ...
  test('verify user can browse content by category', async ({ page }) => {
    // Step 1: Navigate to the Wiley Online Library homepage
